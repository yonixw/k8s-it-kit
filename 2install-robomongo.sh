...
ZIP_PATH=$(curl -L https://github.com/Studio3T/robomongo/releases/latest | grep -oE '[^"]+\/releases\/[^"]+\.tar.gz' )
EXPAND_PATH=$(curl -L https://github.com/Studio3T/robomongo/releases/latest | grep -oE '[^"]+\/expanded_assets\/[^"]+' )
...
if [[ -z $ZIP_PATH ]]
then
	ZIP_PATH=$(curl $EXPAND_PATH  | grep -oE '[^"]+release[^"]+\.tar.gz')
fi
